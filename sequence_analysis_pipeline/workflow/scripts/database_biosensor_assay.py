...
with DatabaseInterfaceSequences(path=database_path) as db:
    sequences_info = db.query(
        f"SELECT id, sequence, fold_change, fold_change_standard_error FROM {TABLE_NAME} WHERE fold_change IS NOT NULL", fetchall=True)
...
with DatabaseInterfaceSequences(path=database_path) as db:
    for sequence_info in uniq_sequences_info:
        ...
        if seq_fold_change > 3.0 and p_value < 1/N:
            ...
            sequence_set.add((sequence_info[1], seq_fold_change, p_value))
            # if p_value < alpha:
            # count += 1
