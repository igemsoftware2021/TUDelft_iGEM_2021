...
def pear_assembly(outputfilename, pear_directory, data_directory):
    ...
    bashfile_name = 'pearfrompy_{time}.sh'.format(time=int(time.time()))
    bashfile_directory = os.path.join(
        'sequence_analysis_pipeline', 'bash_scripts', bashfile_name)
...
def ngmerge_assembly(outputfilename, ngmerge_directory, data_directory):
    with open(os.path.join(data_directory, 'R1_files_processed.txt'), 'r') as rf:
        lines = rf.readlines()
        R1_files_processed = set()
        for line in lines:
    ...
    bashfile_name = 'ngmergefrompy_{time}.sh'.format(time=int(time.time()))
    bashfile_directory = os.path.join(
        'sequence_analysis_pipeline', 'bash_scripts', bashfile_name)
    ...
    with open(bashfile_directory, 'a') as bashfile:
        for i in range(len(R1_list)):
            ...
            if i < len(R2_list):
                ...
                if R1_Inum == R2_Inum:
                    cmdstr = ngmerge_directory + ' -1 ' + \
                        R1_list[i] + ' -2 ' + R2_list[i] + ' -o ' + outputfilename + '_I' + \
                        R1_Inum + ' > ' + outputfilename + '_I' + R1_Inum + '_ngmerge.log'
                    bashfile.write(cmdstr + '\n')
                else:
                    print(
                        f'R1_Inum = {R1_Inum} and R2_Inum = {R2_Inum} -> R1_Inum != R2_Inum\nThe R1_filename is {R1_list[i]}')
...
def sort_results():
    bashfile_directory = os.path.join(
        'sequence_analysis_pipeline', 'bash_scripts', 'readcounts.sh')
    subprocess.call(
        'chmod +x ' + bashfile_directory, shell=True)
    subprocess.call(bashfile_directory, shell=True)


def sort_results_ngmerge():
    bashfile_directory = os.path.join(
        'sequence_analysis_pipeline', 'bash_scripts', 'readcounts_ngmerge.sh')
    subprocess.call(
        'chmod +x ' + bashfile_directory, shell=True)
    subprocess.call(bashfile_directory, shell=True)
...
if __name__ == '__main__':
    outputfilename = 'N41-I14_S14'
    pear_directory = '/home/boydpeters/pear-0.9.11-linux-x86_64/bin/pear'
    ngmerge_directory = '/home/boydpeters/NGmerge-master/NGmerge'
    data_directory = os.path.join('sequence_analysis_pipeline', 'data', 'NGS')
    # pear_assembly(outputfilename, pear_directory, data_directory)
    # ngmerge_assembly(outputfilename, ngmerge_directory, data_directory)
    # sort_results()
    sort_results_ngmerge()
